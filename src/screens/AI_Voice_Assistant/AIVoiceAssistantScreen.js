...
export default function AIVoiceAssistantScreen() {
  ...
  const conversationFlow = [
    {
      id: 1,
      simulatedInput: "app kon ho",
      response:
        "मेरा नाम स्मृति AI असिस्टेंट है। मैं आपकी स्वास्थ्य और दैनिक जरूरतों में सहायता करता हूं।",
      language: "hi",
    },
    {
      id: 2,
      simulatedInput: "meri heart rate bataao",
      response:
        "आपकी दिल की धड़कन 72 बीट प्रति मिनट है। यह सामान्य श्रेणी में है और आपका स्वास्थ्य अच्छा दिख रहा है।",
      language: "hi",
    },
    {
      id: 3,
      simulatedInput: "main mera caregiver ko call lagao",
      response:
        "आपके देखभालकर्ता को कॉल लगाई जा रही है। कृपया थोड़ा इंतजार करें।",
      language: "hi",
      action: "CALL_CAREGIVER",
    },
    {
      id: 4,
      simulatedInput: "thank you",
      response:
        "आपका बहुत-बहुत धन्यवाद! मैं हमेशा आपकी सेवा में हूं। स्वस्थ रहें और खुश रहें।",
      language: "hi",
      isThankYou: true,
    },
  ];
        ...
        try {
          const granted = await PermissionsAndroid.request(
            PermissionsAndroid.PERMISSIONS.CALL_PHONE,
            {
              ...
              message:
                "This app needs access to make phone calls to your caregiver",
  ...
  const startRecording = async () => {
    if (showThankYou || autoProcessing) return;
  ...
  // Process current question based on counter
  const processCurrentQuestion = async () => {
    const currentQuestion = conversationFlow[questionCounter];
    ...
    // Show the simulated input first
    setText(currentQuestion.simulatedInput);
    setDetectedLanguage(currentQuestion.language);

    // After a brief moment, show the response
    setTimeout(async () => {
      setText(currentQuestion.response);
      setLoading(false);
      setAIResponse(true);

      // Handle special actions
      if (currentQuestion.action === "CALL_CAREGIVER") {
        await handleCaregiverCall();
      }

      // Check if it's thank you page
      if (currentQuestion.isThankYou) {
        setShowThankYou(true);
      }

      await speakText(currentQuestion.response);
    }, 1000);
  };

  // Handle caregiver call - Actually make the call
  const handleCaregiverCall = async () => {
    try {
      const caregiverNumber = "+919876543210"; // Replace with actual caregiver number
      const phoneUrl = `tel:${caregiverNumber}`;

      if (Platform.OS === "android") {
        const canCall = await Linking.canOpenURL(phoneUrl);
        if (canCall) {
          console.log("Making caregiver call to:", caregiverNumber);
          await Linking.openURL(phoneUrl); // This will actually make the call
        } else {
      ...
      Alert.alert("Call Error", "Failed to make call to caregiver");
    }
  };

  // Enhanced text-to-speech with automatic progression
  const speakText = async (text) => {
    setAISpeaking(true);

    const isHindi = detectedLanguage === "hi" || /[\u0900-\u097F]/.test(text);
  ...
  // Handle what happens after speech is complete
  const handleSpeechComplete = () => {
    if (showThankYou) {
      // Stay on thank you page, don't progress further
      return;
    }

    // Auto progress to next question after 3 seconds
    setTimeout(() => {
      if (questionCounter < conversationFlow.length - 1) {
  ...
  const regenerateResponse = async () => {
    ...
    if (currentQuestion) {
      ...
      setTimeout(() => {
        setText(currentQuestion.response);
        setLoading(false);
        speakText(currentQuestion.response);
      }, 1000);
    }
  };

  // Get current display text - only show responses, no prompts
  const getCurrentDisplayText = () => {
    ...
    return "नमस्ते! मैं आपका SMRITI AI असिस्टेंट हूं। कुछ भी पूछने के लिए माइक दबाएं।";
  };
  ...
  return (
    ...
    >
      ...
      {/* Language indicator - only show when language is detected */}
      {detectedLanguage && AIResponse && (
        <View
          style={{
            position: "absolute",
            top: verticalScale(50),
        ...
        >
          <Text style={{ color: "#fff", fontSize: scale(10) }}>
                ...
                {AIResponse ? (
                  ...
                  <TouchableOpacity
                    style={{
                      width: scale(110),
                      height: scale(110),
                      backgroundColor: showThankYou ? "#4CAF50" : "#fff",
                      flexDirection: "row",
                      alignItems: "center",
                    ...
                    onPress={showThankYou ? resetAssistant : startRecording}
                    disabled={autoProcessing}
                  >
                    {showThankYou ? (
                      <AntDesign name="check" size={scale(50)} color="#fff" />
                    ) : (
                      <FontAwesome
                        name="microphone"
                        size={scale(50)}
      ...
      {/* Simplified Progress Bar - hidden from user view */}
      <View
        style={{
          position: "absolute",
          bottom: -10, // Hidden below screen
          left: scale(30),
          right: scale(30),
          height: scale(4),
      ...
      >
        <View
          style={{
            width: `${
              ((questionCounter + (showThankYou ? 1 : 0)) /
...
//         // "https://smriti-ai-server.onrender.com/api/v1/ai/transcribe",
//         {
//           audio_data: uri,
//           options: {
...
//         return sendAudioToMyWhisperModel(uri, retries - 1);
//       }
