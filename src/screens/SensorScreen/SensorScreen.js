...
const ESP32Dashboard = () => {
  ...
  // Mock data - replace with your WebSocket data
  const [devices, setDevices] = useState([
    {
      ...
      batteryLevel: 85,
      sensors: {
        heartRate: { enabled: true, value: 72, unit: "bpm" },
        gyroscope: {
          enabled: true,
          value: { x: 0.1, y: -0.2, z: 9.8 },
          unit: "m/s²",
        },
        microphone: { enabled: false, value: 45, unit: "dB" },
        gps: {
          enabled: true,
          value: { lat: 28.4595, lng: 77.0266 },
          unit: "coords",
        },
      },
    },
    {
      ...
      sensors: {
        ...
        gyroscope: { enabled: true, value: { x: 0, y: 0, z: 0 }, unit: "m/s²" },
        microphone: { enabled: true, value: 0, unit: "dB" },
  ...
  const renderSensorValue = (sensor, sensorName) => {
    ...
    switch (sensorName) {
      ...
      case "gyroscope":
        return `X: ${sensor.value.x.toFixed(1)}, Y: ${sensor.value.y.toFixed(
          1
        )}, Z: ${sensor.value.z.toFixed(1)}`;
      case "microphone":
        return `${sensor.value} ${sensor.unit}`;
      case "gps":
    ...
    >
      ...
      <View style={styles.deviceStats}>
        <View style={styles.statItem}>
          ...
          <Text
            style={[styles.statText, { color: getStatusColor(device.status) }]}
          >
            {device.status}
          </Text>
        </View>
        ...
        <View style={styles.sensorHeader}>
          <View style={styles.sensorInfo}>
            ...
            <Text style={styles.sensorName}>
              {sensorName.charAt(0).toUpperCase() +
                sensorName.slice(1).replace(/([A-Z])/g, " $1")}
            </Text>
          </View>
          <TouchableOpacity
            style={[
              styles.toggleButton,
              { backgroundColor: sensor.enabled ? "#3B82F6" : "#E5E7EB" },
            ]}
          ...
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>
              {selectedSensor?.name?.charAt(0).toUpperCase() +
                selectedSensor?.name?.slice(1).replace(/([A-Z])/g, " $1")}{" "}
              Sensor
            </Text>
          ...
          <View style={styles.modalBody}>
            ...
            <View style={styles.dataDisplay}>
              <Text style={styles.dataValue}>
                {selectedSensor &&
                  renderSensorValue(selectedSensor, selectedSensor.name)}
              </Text>
              <Text style={styles.dataUnit}>Current Reading</Text>
            </View>
        ...
        <ScrollView style={styles.content}>
          <View style={styles.deviceOverview}>
            ...
            <View style={styles.overviewCard}>
              <Zap
                size={32}
                color={getBatteryColor(selectedDevice.batteryLevel)}
              />
          ...
          <View style={styles.sensorsSection}>
            ...
            {Object.entries(selectedDevice.sensors).map(
              ([sensorName, sensor]) => (
                <SensorCard
                  key={sensorName}
